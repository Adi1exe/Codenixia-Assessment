...
export default function Chatbot() {
  ...
  return (
    <div className="flex flex-col h-[650px] glass rounded-2xl overflow-hidden shadow-2xl shadow-black/50 transition-all hover:shadow-primary/5">
      {/* Chat Header */}
      <div className="p-5 border-b border-white/10 bg-white/5 flex items-center justify-between backdrop-blur-md">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-xs text-gray-400 font-medium">Powered by Advanced LLM</p>
          </div>
        </div>
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto p-6 space-y-6 scroll-smooth">
          ...
          <div key={i} className={`flex gap-4 ${msg.role === 'user' ? 'flex-row-reverse' : 'flex-row'} animate-slide-up group`}>
            <div className={`w-10 h-10 rounded-full flex items-center justify-center shrink-0 shadow-lg ${
              msg.role === 'user' 
                ...
                : 'bg-surface border border-white/10 text-primary-light'
            }`}>
            ...
            <div className={`p-4 rounded-2xl max-w-[80%] text-[15px] leading-relaxed shadow-sm transition-all ${
              msg.role === 'user' 
                ...
                : 'bg-white/5 text-gray-200 border border-white/10 rounded-tl-sm hover:border-white/20'
            }`}>
              {msg.role === 'assistant' ? (
                <div className="prose prose-invert prose-sm max-w-none prose-p:leading-relaxed prose-pre:bg-black/50 prose-pre:border prose-pre:border-white/10 prose-a:text-primary-light">
        ...
        {loading && (
          <div className="flex gap-4 flex-row animate-fade-in">
             <div className="w-10 h-10 rounded-full flex items-center justify-center shrink-0 bg-surface border border-white/10 text-primary-light shadow-lg">
              <Bot size={20} />
            </div>
            <div className="p-4 rounded-2xl bg-white/5 text-gray-400 border border-white/10 rounded-tl-sm flex gap-3 items-center">
      ...
      <div className="p-5 border-t border-white/10 bg-black/20 backdrop-blur-md">
        <form onSubmit={handleSend} className="relative group">
          <input 
            ...
            className="w-full bg-white/5 border border-white/10 hover:border-white/20 rounded-xl pl-5 pr-14 py-4 text-sm text-white focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary transition-all placeholder:text-gray-500 shadow-inner"
          />
          <button 
            type="submit" 
